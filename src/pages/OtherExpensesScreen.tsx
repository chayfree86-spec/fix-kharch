...
import { OtherExpenseItem } from '../types';
...
export const OtherExpensesScreen: React.FC = () => {
  const { selectedMonthData, addOtherExpense, updateOtherExpense, deleteOtherExpense, summary } = useApp();

  const [searchQuery, setSearchQuery] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<OtherExpenseItem | null>(null);
  ...
  const filteredExpenses = useMemo(() => {
    if (!searchQuery.trim()) return selectedMonthData.otherExpenses;
    const q = searchQuery.toLowerCase();
    return selectedMonthData.otherExpenses.filter(item => item.name.toLowerCase().includes(q));
  }, [selectedMonthData.otherExpenses, searchQuery]);
  ...
  const handleOpenEdit = (item: OtherExpenseItem) => {
    setEditingItem(item);
    setName(item.name);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setFormError('Please enter the expense name');
      return;
    }
    if (amount <= 0) {
    ...
    if (editingItem) {
      updateOtherExpense(editingItem.id, {
        name: name.trim(),
        amount,
      });
    } else {
  ...
  const handlePromptDelete = (item: OtherExpenseItem) => {
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="bg-cream rounded-card p-4 sm:p-5 border border-border-warm shadow-warm-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-accent-red text-cream flex items-center justify-center shadow-sm">
            <ReceiptText className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-coffee">Other Expenses</h2>
            <p className="text-xs text-caramel">
              {selectedMonthData.otherExpenses.length} miscellaneous & ad-hoc expenses for {selectedMonthData.monthName}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
        <SearchInput
          ...
          placeholder="Search other expense..."
      ...
      {filteredExpenses.length === 0 ? (
        <EmptyState
          icon={ReceiptText}
          title={searchQuery ? 'No matching expense found' : 'No other expenses recorded'}
          description={
            searchQuery
              ? `No expense matches "${searchQuery}".`
              : 'Add miscellaneous café expenses like packaging, repairs, coffee beans, or supplies.'
          }
          actionText={searchQuery ? undefined : 'Add Other Expense'}
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-lg bg-warm-beige text-accent-red flex items-center justify-center flex-shrink-0">
      ...
      {/* Add / Edit Other Expense Modal */}
      <Modal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        title={editingItem ? 'Edit Other Expense' : 'Add Other Expense'}
        subtitle="Miscellaneous café cost item"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="e.g. Packaging, Cleaning Supplies, Maintenance"
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to remove ${deletingName} from this month's miscellaneous expenses?`}
